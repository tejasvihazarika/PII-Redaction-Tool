...
REGEX_PATTERNS = {
    ...
    "Phone": r'\b[6-9]\d{9}\b|(?:\+?\s*91\s*[-.\s]?|0)[\s-]?\(?\d{2,5}\)?[\s.-]?\d{3,5}[\s.-]?\d{3,5}\b|\b022[-]\d{8}\b|\b\+91[-]\d{2,4}-\d{5,8}\b',
    "SSN": r'\b\d{3}-\d{2}-\d{4}\b|\bSSN:?\s*\d{9}\b|(?:SAP\s*ID|Student\s*ID|Roll\s*No)[:\s]*(\d{6,12})\b',
...
def detect_pii(text: str) -> List[Dict[str, Any]]:
    ...
    # ── Step 1: Ground-truth lookup pass ──────────────────────────────────
    ...
    for pii_type, pattern in REGEX_PATTERNS.items():
        for match in re.finditer(pattern, text, re.IGNORECASE):
            matched_str = match.group(1) if (match.groups() and match.group(1) is not None) else match.group(0)
            orig = matched_str.strip()
            start = match.start(1) if (match.groups() and match.group(1) is not None) else match.start()
            end = match.end(1) if (match.groups() and match.group(1) is not None) else match.end()
